...
- 자코비안 참고 내용    
	...
	- 여기서, 사용되는 개념이 Jacobian인데, **‘반구의 각도→ 광원 표면 면적’으로 투영**시킨 것이다. 그러면,  또 하나 의문이 생기는 것은 ‘왜 $cos\theta_o$가 등장하는 것인가?’ 평면으로 **투영된 곳의 면적**은 **원래 넓이**에서 $|\vec{n} \cdot \vec{\omega}|$를 곱한 값이어야 하기 때문이다.
	$$
